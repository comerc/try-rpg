...
async function main() {
  ...
  for (const team of teams) {
    const atlas = path.join(atlasDir, `footman-${team}-8x8-grid.png`);
    ...
    for (const rowState of rowStates) {
      ...
      for (let i = 0; i < sourceFrames.length; i++) {
        await fs.writeFile(path.join(sourceStateDir, `${String(i).padStart(3, '0')}.png`), sourceFrames[i]);
      }
      ...
      for (const output of rowState.outputs) {
        ...
        for (let frame = 0; frame < output.count; frame++) {
          ...
          const png = sourceFrames[sourceIndex];
          const frameName = `${String(frame).padStart(3, '0')}.png`;
          const framePath = path.join(stateDir, frameName);
          const runtimePath = path.join(outDir, `unit-footman-${team}-${output.state}-${frame}.png`);
  ...
  console.log(`applied ${written.length} runtime footman files from 8x8 asset-rules atlases`);
}
